...
class Tempbans {
    ...
    async get (guildID, userID) {
        ...
        return new class Tempban {
            ...
            async remove () {
                return await r.table("Tempbans").get(tb[0].id).delete().run(this.conn);
            }
        };
    }
    ...
    async interval () {
        ...
        const obj = [];
        const arr = await r.table("Tempbans").between(1, Date.now(), { index: "endDate" }).coerceTo("array").run(this.conn);
        ...
        arr.forEach(tb => {
            obj.push(new class Tempban {
                ...
                async remove () {
                    return await r.table("Tempbans").get(tb.id).delete().run(conn);
                }
            });
        });
