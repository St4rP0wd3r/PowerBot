...
module.exports = class Command extends BaseCommand {
    constructor() {
        super({
            name: "crime",
            aliases: ["obrabuj", "cygan"],
            description: "Okrada bank.",
            usage: "<p>crime",
            category: "Ekonomia",
            disabled: true,
        });
    ...
    async run(message) {
        ...
        if (user.cooldown.crime > Date.now()) {
            return sender.error("Odczekaj `" + timing(user.cooldown.crime - Date.now()) + "` przed następnym rabunkiem.");
        }

        r.table("Economy").getAll([message.guild.id, message.author.id], { index: "getAll" }).update({ cooldown: { crime: Date.now() + settings.economy.cooldown.crime } }).run(client.database.conn);
        ...
        if (!random) {
            ...
            return sender.error(settings.economy.replies.crime.fail[Math.floor(Math.random() * settings.economy.replies.crime.fail.length)].replace(/{cash}/g, moneySub + settings.economy.vault));
        }
        ...
        sender.create(settings.economy.replies.crime.success[Math.floor(Math.random() * settings.economy.replies.crime.success.length)].replace(/{cash}/g, moneyAdd + settings.economy.vault));
    }
};
